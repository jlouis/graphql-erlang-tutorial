...
The first query we will run requests a given *Starhip* from the system.
In accordance with the rules of Relay Modern GraphQL conventions, we
will load this *Starship* via the _Object Identification_ conventions
of Relay Modern:

[source,graphql]
----
query StarShipQuery {
    node(id: "U3RhcnNoaXA6AAAAAAAAAAI=") { # <1>
      ... on Starship { # <2>
        id # <3>
        name
        cargoCapacity
...
<2> This notation, if you are only slightly familiar with GraphQL is
    ...
    type *Node* and here we restrict ourselves to the type *Starship*.
