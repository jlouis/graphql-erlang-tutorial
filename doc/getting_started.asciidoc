...
into appropriate mnesia records. The starship is split over two
tables: one for transports and one for starships. We handle the
transport part first, and then we patch the data with the starship
information later.

[source,erlang]
----
include::{sw_core}/src/sw_core_db.erl[tags=jsonToStarship]
----
...
[source,erlang]
----
include::{sw_core}/src/sw_core_db.erl[tags=populateStarships]
----
