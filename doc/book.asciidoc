...
May 29th, 2017:: Moved <<cqrs>> into terminology so it can be referenced
  from other places in the document easily. Described
  <<schema-default-values>>.  Described <<middleware-stacks>>. The
  <<system-tour>> now includes simple mutations as an example.
